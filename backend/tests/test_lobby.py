...
from backend.lobby.dependencies import get_lobbies, get_random_code
...
def test_create_game(client):
    ...
    assert client.get(f"/lobbies/AAAA").raise_for_status().json() == lobby
...
def test_second_player_in_game_after_join(player_1: TestClient, player_2: TestClient):
    ...
    player_2.post(f"/lobbies/AAAA/join", json={"username": "player 2"}).raise_for_status()
    assert player_2.get(f"/lobbies/AAAA").json()['players'] == ["player 1", "player 2"]
...
def test_switch_from_lobbies(player_1: TestClient, player_2: TestClient):
    ...
    player_1.post("/lobbies", json=get_testgame_config()).json()
    response = player_2.post(f"/lobbies/AAAB/join", json={"username": "player 2"}).raise_for_status()
    assert response.status_code == 204


def test_creating_two_lobbies_deletes_the_first_one(player_1: TestClient, lobbies: dict[str, Lobby]):
    player_1.post("/lobbies", json=get_testgame_config()).raise_for_status()
    player_1.post("/lobbies", json=get_testgame_config()).raise_for_status()
...
def test_player_2_switches_between_lobbies_will_remove_it_from_the_first_one(
    ...
    player_2.post(f"/lobbies/AAAA/join", json={"username": "player 2"}).raise_for_status()
    player_2.post(f"/lobbies/AAAB/join", json={"username": "player 2"}).raise_for_status()
